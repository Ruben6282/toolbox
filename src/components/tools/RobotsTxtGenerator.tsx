...
import { Copy, Download, RotateCcw, Bot, AlertCircle } from "lucide-react";
...
export const RobotsTxtGenerator = () => {
  ...
  const [userAgents, setUserAgents] = useState([
    { name: "*", allow: true, disallow: [] as string[], crawlDelay: 0 }
  ]);
  ...
  const addDisallowPath = (userAgentIndex: number) => {
    ...
    if (agent.disallow.length >= SECURITY_LIMITS.MAX_DISALLOW_PATHS_PER_AGENT) {
      notify.error(
        `Maximum ${SECURITY_LIMITS.MAX_DISALLOW_PATHS_PER_AGENT} paths per user agent allowed`
      );
      return;
    }
    const updated = [...userAgents];
  ...
  const generateRobotsTxt = () => {
    ...
    userAgents.forEach((agent) => {
      ...
      agent.disallow.forEach((path) => {
        ...
        if (trimmedPath) {
          ...
          const safePath = truncateText(
            trimmedPath,
            SECURITY_LIMITS.MAX_PATH_LENGTH
          );
      ...
      if (agent.crawlDelay > 0) {
        const safeDelay = Math.min(
          Math.max(0, agent.crawlDelay),
          SECURITY_LIMITS.MAX_CRAWL_DELAY
        );
    ...
    if (customRules.trim()) {
      ...
      const safeCustomRules = truncateText(
        customRules,
        SECURITY_LIMITS.MAX_CUSTOM_RULES_LENGTH
      )
        // eslint-disable-next-line no-control-regex
    ...
    // Add AI crawler blocking section if enabled
    if (blockAICrawlers) {
    ...
    <div className="space-y-6">
      <Card>
        ...
        <CardContent className="space-y-4">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            <div className="space-y-2">
              ...
              <Input
                ...
                aria-describedby={
                  validationErrors.sitemapUrl ? "sitemap-error" : "sitemap-hint"
                }
              />
          ...
          {/* AI crawler blocking toggle */}
          <div className="flex items-center space-x-2">
            <Checkbox
              id="block-ai"
              checked={blockAICrawlers}
              onCheckedChange={(checked) => setBlockAICrawlers(!!checked)}
            />
            <Label htmlFor="block-ai">
              Block common AI crawlers (GPTBot, Google-Extended, Claude-Web,
              PerplexityBot, etc.)
            </Label>
          </div>
            ...
            <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-2">
              <div>
                ...
                <p className="text-xs text-muted-foreground mt-1">
                  {userAgents.length}/{SECURITY_LIMITS.MAX_USER_AGENTS} user
                  agents
                </p>
              </div>
              <Button
                    ...
                    <div className="flex items-center space-x-2">
                      <Checkbox
                        ...
                        onCheckedChange={(checked) =>
                          updateUserAgent(index, "allow", !!checked)
                        }
                      />
                ...
                <div className="space-y-2">
                  <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-2">
                    <div>
                      ...
                      <p className="text-xs text-muted-foreground mt-1">
                        {agent.disallow.length}/
                        {SECURITY_LIMITS.MAX_DISALLOW_PATHS_PER_AGENT} paths
                      </p>
                    </div>
                    <Button
                      ...
                      disabled={
                        agent.disallow.length >=
                        SECURITY_LIMITS.MAX_DISALLOW_PATHS_PER_AGENT
                      }
                    >
                      Add Path
                    </Button>
                  ...
                  {agent.disallow.map((path, pathIndex) => (
                    <div key={pathIndex} className="flex flex-col gap-2">
                      <div className="flex flex-col sm:flex-row gap-2">
                        <Input
                          ...
                          onChange={(e) => {
                            ...
                            setValidationErrors((prev) => {
                              const newErrors = { ...prev };
                              delete newErrors[
                                `userAgent_${index}_path_${pathIndex}`
                              ];
                              return newErrors;
                            });
                          }}
                        ...
                        <Button
                          onClick={() =>
                            removeDisallowPath(index, pathIndex)
                          }
                          variant="outline"
                          size="sm"
                          className="w-full sm:w-auto"
          ...
          <div className="space-y-2">
            <div className="flex items-center justify-between">
              ...
              <p className="text-xs text-muted-foreground">
                {customRules.length}/{SECURITY_LIMITS.MAX_CUSTOM_RULES_LENGTH}{" "}
                characters
              </p>
            </div>
            <Textarea
              ...
              aria-describedby={
                validationErrors.customRules ? "custom-rules-error" : undefined
              }
            />
